...
export default function Footer() {
  ...
  const contactNumbers = [
    {
      label: t('footer.primary_number') || 'Owner / Direct',
      display: BRAND_PRIMARY_PHONE_DISPLAY,
      href: `tel:${BRAND_PRIMARY_PHONE_E164}`,
    },
    {
      label: t('footer.secondary_number') || 'WhatsApp / Support',
  ...
  const remedies = useMemo(() => {
    ...
    return derived.length > 0
      ? derived.slice(0, 6)
      : ['Casual Wear', 'Formal Suits', 'Traditional Wear', 'Festive Collection', 'Ethnic Wear', 'Wedding Special']
  }, [products])

  return (
    <footer className="bg-[#081B40] text-gray-300 mt-12 sm:mt-16 border-t border-[#D4AF37]/30">
      <div className="max-w-7xl mx-auto px-4 py-10 sm:py-14 grid grid-cols-1 md:grid-cols-4 gap-8 sm:gap-10">
        <div className="md:col-span-1">
          <div className="flex items-center gap-2.5 mb-4">
            <div className="w-10 h-10 bg-[#0B2559] border border-[#D4AF37]/60 rounded-xl flex items-center justify-center p-1 overflow-hidden shrink-0">
            ...
            <div>
              ...
              <p className="text-[10px] uppercase tracking-[0.2em] text-[#D4AF37] font-bold">{BRAND_SUBTITLE}</p>
            </div>
          </div>
          <p className="text-sm text-gray-300 leading-relaxed mb-4">{t('footer.desc')}</p>
          <p className="text-xs text-[#D4AF37] font-bold uppercase tracking-wider">{t('footer.tags')}</p>
        </div>
        <div>
          <h5 className="text-white font-bold mb-4 sm:mb-5 uppercase text-xs tracking-widest">{t('footer.shop')}</h5>
          <ul className="flex flex-col gap-2.5 text-sm">
            {categories.map(c => (
              <li key={c}><Link to={`/products?cat=${encodeURIComponent(c)}`} className="hover:text-[#D4AF37] transition-colors">{t('cat.' + c)}</Link></li>
            ))}
          </ul>
        </div>
        <div>
          ...
          <ul className="flex flex-col gap-2.5 text-sm">
            {remedies.map(r => (
              <li key={r}><Link to={`/products?remedy=${encodeURIComponent(r)}`} className="hover:text-[#D4AF37] transition-colors">{t('remedy.' + r)}</Link></li>
            ))}
          </ul>
        </div>
        <div>
          ...
          <ul className="flex flex-col gap-4 text-sm">
            <li className="flex items-start gap-3">
              ...
              <div className="flex flex-col gap-1">
                ...
                <span className="text-[11px] leading-relaxed text-gray-400 max-w-[260px]">{BRAND_ADDRESS}</span>
              </div>
            </li>
            {contactNumbers.map((contact) => (
              <li key={contact.label} className="flex items-start gap-3">
                ...
                <div className="flex flex-col gap-0.5">
                  <span className="text-[11px] uppercase tracking-[0.18em] text-gray-400">{contact.label}</span>
      ...
      <div className="border-t border-white/10 py-5 text-center text-xs space-y-1.5">
        <div className="text-gray-300 font-medium">{t('footer.rights')}</div>
        <div className="text-gray-300 font-bold">{t('footer.powered_by')}</div>
      </div>
    </footer>
  )
}
