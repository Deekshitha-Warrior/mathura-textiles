...
import { BRAND_EN, BRAND_SUBTITLE, BRAND_TAGLINE, BRAND_WHATSAPP, BRAND_ICON } from '../lib/brand'
...
export default function Navbar() {
  ...
  return (
    <>
      <div className="flex flex-wrap items-center justify-center gap-x-3 gap-y-0.5 bg-[#0B2559] px-3 py-2 text-center text-[11px] sm:text-xs font-semibold tracking-wide text-white border-b border-[#D4AF37]/30 shadow-soft">
        <span className="leading-snug">
          ✨ {BRAND_SUBTITLE} &nbsp;·&nbsp; <span className="whitespace-nowrap text-[#D4AF37]">WhatsApp: {BRAND_WHATSAPP}</span>
        </span>
        <button onClick={() => setLang(lang === 'en' ? 'ta' : 'en')} className="hidden sm:inline-flex items-center rounded-full bg-white/10 p-0.5 text-[10px] font-bold text-white transition-colors shadow-soft ml-2">
          <span className={`px-2 py-0.5 rounded-full transition-colors ${lang === 'en' ? 'bg-[#D4AF37] text-[#0B2559] font-black' : 'text-white/80'}`}>EN</span>
          <span className={`px-2 py-0.5 rounded-full transition-colors ${lang === 'ta' ? 'bg-[#D4AF37] text-[#0B2559] font-black' : 'text-white/80'}`}>தமிழ்</span>
        </button>
      </div>

      <header className="sticky top-0 z-40 glass border-b border-[#E2E8F0] shadow-sm">
        <nav className="mx-auto flex max-w-7xl items-center justify-between gap-2 sm:gap-3 px-3 py-2.5 sm:px-4 sm:py-3 lg:gap-4">
          <Link to="/" className="group flex min-w-0 items-center gap-2 sm:gap-2.5">
            <div className="w-10 h-10 sm:w-11 sm:h-11 bg-[#0B2559] rounded-xl flex items-center justify-center overflow-hidden shadow-sm shrink-0 border border-[#D4AF37]/60 group-hover:opacity-90 transition-opacity p-1">
              <img src={BRAND_ICON} alt={BRAND_EN} className="w-full h-full object-contain" />
            </div>
            <div className="flex min-w-0 flex-col leading-none">
              <p className="break-words text-[13px] sm:text-[14px] font-black leading-tight tracking-tight text-[#0F172A] md:text-[16px] font-headline">{BRAND_EN}</p>
              <p className="hidden text-[9px] font-bold uppercase tracking-[0.15em] text-[#B38018] md:block">{BRAND_TAGLINE}</p>
            </div>
          </Link>
