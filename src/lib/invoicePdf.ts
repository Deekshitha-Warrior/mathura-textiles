...
export function createInvoicePdf(data: InvoicePdfData): Blob {
  ...
  const left = 16
  const right = 194
  const primaryColor = '#0B2559' // Deep Royal Navy
  const goldColor = '#B38018' // Deep Metallic Gold
  const ink = '#0F172A'
  const muted = '#475569'
  let y = 16
  ...
  try {
    doc.addImage(LOGO_BASE64, 'PNG', left, y, 22, 22)
  } catch {
  ...
  doc.text('PREMIUM WHOLESALE STORE ("Our Suit, Your Style")', left + 26, y + 10)
  doc.setFontSize(8)
  doc.setTextColor(muted)
  doc.setFont('helvetica', 'normal')
  doc.text(BRAND_ADDRESS, left + 26, y + 15, { maxWidth: 85 })
  doc.text(`Phone: ${BRAND_PHONE_DISPLAY}`, left + 26, y + 24)
