...
const SRC_ICON = path.join(PUBLIC_DIR, 'mathura-logo.jpeg');
...
async function main() {
  ...
  for (let y = 0; y < info.height; y++) {
    for (let x = 0; x < info.width; x++) {
      ...
      const g = data[idx + 1];
      const b = data[idx + 2];
      // Check if pixel belongs to logo (not background)
  ...
  await generateSquareIcon(192, 155, path.join(PUBLIC_DIR, 'mathura-icon-192.png'));
